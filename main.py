...
def bfs(origin, target, image_path):
    video_maker = VideoMaker("BFS_Visualization", image_path)
    queue = [origin]
    visited = set()
    ...
    while queue:
        ...
        for direction in directions:
            ...
            if (
                0 <= neighbor[0] < video_maker.frame.shape[1]
                and 0 <= neighbor[1] < video_maker.frame.shape[0]
                and neighbor not in visited
            ):
                visited.add(neighbor)
                queue.append(neighbor)
                video_maker.change_pixel(
                    neighbor[0], neighbor[1], (255, 0, 0)
                )  # Mark as visited
    video_maker.release()
